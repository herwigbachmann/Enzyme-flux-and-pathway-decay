...
file_path <- "Data/20190424-layout trial 1 transition 1.txt" 

# --- 2. Read the relevant data from the .txt file ---
raw_data <- read_delim(
  file_path,
  delim = "\t", # Assuming tab-separated. Adjust if it's comma (","), space (" "), etc.
...
for (i in seq_along(descriptor_rows_indices)) {
  ...
  colnames(well_data_block) <- as.character(1:24) 
